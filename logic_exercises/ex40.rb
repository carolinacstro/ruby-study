=begin    

Faça um algoritmo para ler: a descrição do produto (nome), a quantidade
adquirida e o preço unitário. Calcular e escrever o total
(total = quantidade adquirida * preço unitário), o desconto e o total a
pagar (total a pagar = total - desconto), sabendo-se que:

- Se quantidade <= 5 o desconto será de 2%
- Se quantidade > 5 e quantidade <=10 o desconto será de 3% 
- Se quantidade > 10 o desconto será de 5%

=end 

print "Digite o nome do produto: "
nome_produto = gets.chomp

print "Quantidade adquirida: "
quant_adquirida = gets.chomp

print "Preço da unidade: "
preco_unidade = gets.chomp 

total = quant_adquirida * preco_unidade

if quant_produto <= 5
    desconto = total/100 * 2
    total_a_pagar = total - desconto
    puts total_a_pagar
elsif quant_adquirida > 5 && <= 10
    desconto = total/100 * 3
    total_a_pagar = total - desconto
    puts total_a_pagar
else
    desconto = total/100 * 5
    total_a_pagar = total - desconto
    puts total_a_pagar
end

